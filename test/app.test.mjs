...
test("static app includes every primary control and asset reference", async () => {
  ...
  assert.match(script, /const HILL_ROTATIONAL_INERTIA = 5\.4/);
  assert.match(script, /const HILL_MAX_SPIN = 0\.052/);
  assert.match(script, /const HILL_GROUND_TILT = 0\.00025/);
  assert.match(script, /const HILL_AIR_TILT = 0\.00145/);
  assert.match(script, /const HILL_PITCH_STABILITY = 0\.018/);
  assert.match(script, /const HILL_PITCH_DAMPING = 0\.88/);
  assert.match(script, /const SUSPENSION_REST_LENGTH = 42/);
  assert.match(script, /const SUSPENSION_TRAVEL = 28/);
  assert.match(script, /const SUSPENSION_STIFFNESS = 0\.036/);
  assert.match(script, /const SUSPENSION_DAMPING = 0\.34/);
  assert.match(script, /const SUSPENSION_FORCE_LIMIT = 1\.15/);
  assert.match(script, /const ANTI_ROLL_STRENGTH = 0\.0025/);
  ...
  assert.match(script, /const springForce = clamp\(/);
  assert.match(script, /applyAntiRoll\(agent, rear, front\)/);
  assert.match(script, /function applyPitchStability\(agent, rear, front\)/);
  assert.match(script, /agent\.angularVelocity \*= HILL_PITCH_DAMPING/);
  assert.match(script, /applyPitchStability\(agent, rear, front\)/);
